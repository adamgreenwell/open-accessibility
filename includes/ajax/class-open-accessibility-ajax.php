...
class Open_Accessibility_Ajax {
	...
	public static function generate_statement() {
		// Check for nonce
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'open_a11y_nonce' ) ) {
		...
		// Get form data
		$org_name = isset( $_POST['org_name'] ) ? sanitize_text_field( $_POST['org_name'] ) : get_bloginfo( 'name' );
		$contact_email = isset( $_POST['contact_email'] ) ? sanitize_email( $_POST['contact_email'] ) : '';
		$conformance_level = isset( $_POST['conformance_level'] ) ? sanitize_text_field( $_POST['conformance_level'] ) : 'AA';
		$create_page = isset( $_POST['create_page'] ) && $_POST['create_page'] === 'true';
		...
		if ( ! class_exists( 'Open_Accessibility_Statement_Generator' ) ) {
			require_once OPEN_A11Y_PLUGIN_DIR . 'includes/class-open-accessibility-statement-generator.php';
		}
